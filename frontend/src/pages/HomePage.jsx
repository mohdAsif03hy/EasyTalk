...
function HomePage() {
    ...
    useEffect(() => {
        ...
        setOutgoingRequestsIds(outgoingIds); // always update
    }, [outgoingFriendReqs]);
            ...
            <div className="container mx-auto space-y-10">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <Link to="/notifications" className="btn btn-outline btn-sm">
                        <UserIcon className="mr-2 size-4" />Friends Requests
                    </Link>
                </div>
                ...
                <section>
                    ...
                    {loadingUsers ? (
                        ...
                        <div className="card bg-base-200 p-6 text-center">
                            <h3 className="font-semibold text-lg mb-2 ">No recooendetions available</h3>
                        ...
                        <div className="grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {recommendedUsers.map((user) => {
                                ...
                                return (
                                    <div key={user._id} className="card bg-base-200/60 backdrop-blur-md hover:shadow-xl transition-all duration-300 rounded-2xl">
                                        <div className="card-body p-5 space-y-4">
                                            <div className="flex items-center gap-3">
                                                <div className="avatar size-16 rounded-full ring-2 ring-primary/30 ring-offset-2 ring-offset-base-200">
                                            ...
                                            {user.bio && <p className="text-sm opacity-70 leading-relaxed mb-1">{user.bio}</p>}
                                            {/* ACTION MAESSAGE BTN  */}
                                            <button className={`btn w-full mt-2 ${hasRequestBeenSent ? "btn-disabled" : "btn-primary"
                                                }`}
                                                onClick={() => {
                                                    setOutgoingRequestsIds(prev => new Set(prev).add(user._id)); // instant UI disable
                                                    sendRequestMutation(user._id); // backend request
                                                }}
