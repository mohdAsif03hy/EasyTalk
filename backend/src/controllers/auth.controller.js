...
export async function signup(req, res) {
    ...
    try {
        ...
        const idx = Math.floor(Math.random() * 100) + 1; //generate num bt 1 - 100
        ...
        res.cookie("jwt", token, {
            ...
            secure: process.env.NODE_ENV === "production", //HTTp request
        });
        res.status(201).json({ success: true, user: newUser })

    } catch (error) {
        console.log("error in sognup controller ", error);
...
export async function onboard(req, res) {
    try {
        ...
        try {
            ...
            // console.log("errror updating Stream user during onboarding:", streamError.message);
        } catch (streamError) {
            console.log("errro updating stream user during onboarding ", streamError.message);
        }
